...
| `gh` | 경기주택도시공사 | ✅ **API 구현(이력만)** | 사이트는 robots `Disallow: /` 유지 → odcloud API(15119414)로 우회. ⚠️ 연 1회(8월) 스냅샷이라 최신 공고 없음 |
...
### ✅ gh — 공공데이터 API 구현 (2026-07-15, 이력 전용)
- 사이트 직접 크롤은 robots `Disallow: /` 전면 차단이라 제외(존중).
- 대신 공공데이터포털 'GH주택청약 모집정보'(fileData 15119414)를 노출한 **odcloud API** 사용 —
  `GET https://api.odcloud.kr/api/15119414/v1/uddi:d22eef31-…` (2025-08-21판 990건,
  2024판 783건·2023판 1,481건 UDDI 별도). 데이터셋 **활용신청 필요**(2026-07-15 완료).
- 필드: 공고명·게시일자·접수시작/종료일자·당첨자발표일자·입주예정년월·주택관리번호 등.
  2017년부터 이력 제공. 공고문 PDF·상세 URL 은 없음(공고 페이지가 robots 차단 영역).
- ⚠️ **연 1회(8월) 스냅샷 갱신**(차기 2026-08-21) — 최신 공고는 이 채널로 못 받는다.
  GH 최신 임대공고가 myhome 통합 경로로 올라오는지 별도 확인 필요.
- 스냅샷 간 중복은 최신판부터 순회 + 주택관리번호 기반 notice_id 로 엔진이 스킵.
...
## 다음 단계
1. sh_ish·gndc·udc·gh 백필(--since 2021) 실행 및 정기 수집 크론 편입.
2. GH 최신 공고의 myhome 커버 여부 확인(스냅샷 공백 보완).
3. 대전 WAF는 별도 협의/공식 채널 검토. 매년 8월 GH 신규 스냅샷 UDDI 추가.
